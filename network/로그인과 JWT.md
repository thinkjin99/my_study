...
이제 RS256 방식으로 JWT 토큰을 발급 해보자. 우선적으로 개인 키와 공개키를 openssl을 사용해 발급 받는다. 개인 키는 아래와 같이 발급 받을 수 있다.
...
리프레시 토큰은 되도록 DB에 저장해 서버 단에서 관리하는 것이 좋다. 서버 단에서 관리했을 때 발생하는 이점은 다음과 같다.
****
* 리프레시 토큰 회수가 가능해짐
