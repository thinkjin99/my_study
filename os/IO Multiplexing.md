...
## Select의 FD_LIST의 위치는 User mode에 존재 ##
- select 함수에서 사용되는 FD_LIST 는 사용자 모드에 존재함. 이는 사용자 영역에서 FD_LIST가 생성되고 초기화 되어지기 때문임. -> 보통 파일디스크립터는 OS커널에서 처리되어지지만, 현재 언급하고 있는 FD_LIST는 다른 자료 구조임을 명확하게 구분지어야함
		  1. 파일 디스크립터 : 파일에 대한 메타데이터 관리 (커널 영역에서 처리)
		  2. FD_LIST (파일 디스크립터 리스트) : 감시할 파일 디스크립터들을 번호로 매겨놓은 리스트 자료구조 (유저 영역에서 처리)
		  **이유와 장점**
		  - 유연성과 편의성 : 유저모드에 FD_LIST가 존재하기 떄문에 감시하고자 하는 파일 디스크립터를 유연하게 관리 가능(추가 및 제거)
		  - 커널 메모리 절약 : 해당 기술이 설계될 당시 메모리 공간이 많지 않아 커널 메모리 공간을 최소화하는 것이 중요하여 FD_LIST는 유저모드로 꺼내지게 된 
- 해당 FD_LIST는 select 함수 호출시 커널 모드에 전달되어짐
- 커널 모드에서는 리스트를 사용하여 파일의 I/O 상태를 감시하며, 읽기,쓰기, 예외 상태 발생시 select는 이를 반환
...
**epoll은 내가 이벤트를 감지하고 싶은 FD_SET을 특정해 커널에 전달할 수 있다.** 따라서 관심있는 fd의 이벤트만을 감지하는 것이 가능하다. 또한 커널에서 FD_SET(Event Set)을 관리하기 때문에 유저-커널 통신으로 인한 오버헤드가 적은 편이다. epoll은 다음과 같이 동작한다.
...
- 컴퓨터의 하드웨어 발전으로 인해 커널 리소스가 커짐 
- FD_SET을 커널 모드에서 관리할 수 있게 되어짐 
		이유와 장점
		- 감시하는 FD_LIST와 처리를 진행하는 FD가 같은 커널 영역에 존재하므로, **이벤트 발생시 빠른 반응을 할 수 있어짐**
		- 유저모드에 있는 내용을 커널에 복사하는 개념이 아니라 같은 커널 공간에 사용하므로 자원공간의 효율 증가
		- 유저모드와 커널모드가 communication을 진행하는 시스템 콜 수가 줄어듦. select에서 발생하는 FD_LIST가 커지면 시스템 콜 수가 증가하는 문제점을 **epoll에서 FD_LIST를 커널 영역으로 가져옴으로서 더 많은 FD를 관라할 수 있게 되어짐** 
___
### epoll 더 알아보기
...
epoll 인스턴스는 소켓과 흡사하게 커널의 데이터를 유저의 영역으로 들고오는 인터페이스로 동작한다. 따라서 직접 **FD_SET을 돌며 커널에 IO 완료 여부를 물어봐야 했던 select와 달리 유저 영역에 있는 인스턴스의 상태만 체크하면 되므로 epoll이 더 효율적으로 동작한다. (모드 스위칭이 적다)**
...
#### epoll_wait
[epoll_wait](https://man7.org/linux/man-pages/man2/epoll_wait.2.html)는 <span class= "red red-bg"><b>블락킹 함수로 구독한 fd에서 이벤트가 발생하는지 정해진 시간 동안 대기하는 함수이다.</b></span> 이벤트가 발생할 경우 **커널로 부터 전달 받은 evnet 객체를 통해 인스턴스의 ready 리스트에서 이벤트가 발생한 fd를 반환**한다.
