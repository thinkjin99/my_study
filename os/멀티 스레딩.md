...
범수는 혼자 일하지만, 작업 중에 기다려야 하는 시간이 있을 때마다 타이머를 사용하여 효율적으로 시간을 관리한다. 예를 들어, 고기를 굽는 동안 계란을 풀고 설거지를 하며 마늘을 써는 등 **하나의 작업이 완료될 때까지 기다리지 않고 다른 작업을 진행한다.** 범수는 한 가지 작업을 차단(blocking)하지 않고, 여러 작업을 효율적으로 수행하여 싱글 쓰레드의 효율을 극대화한다.
