...
프로세스는 위와 같은 상태들을 가지며 이러한 상태의 변경은 전부 커널에서 이뤄진다. PCB에 대한 접근은 커널 영역에서만 가능하기 때문이다. 이러한 상태들은 스케줄러에서 대표적으로 활용되며 스케줄러는 상태를 확인하고 스케줄링의 우선순위를 설정한다.
...
* **init process**
	<u><b>init process는 리눅스의 부팅중 가장 먼저 실행되는 프로세스로 프로세스 트리의 루트에 위치한 프로세스이다.</b></u> 리눅스의 모든 프로세스는 해당 프로세스의 자식이다. init 프로세스는 고아 프로세스들을 입양하고 정상적으로 종료하는 역할을 수행한다. 입양하는 과정에서 오버헤드가 발생하기 때문에 되도록 입양은 발생하지 않게 하는 것이 좋다.
